...
class EmailMessageFormat : BaseMessageFormat() {
    ...
    override fun counterSubject(event: AlertEvent): String = "[Counter][${event.level}] ${event.objName} - ${event.metric}"

    override fun xlogErrorSubject(event: XlogErrorEvent): String = "[Xlog Error] ${event.objName} - ${event.service}"

    override fun xlogSlowSubject(
        pack: XLogPack,
        thresholdMs: Int,
    ): String = "[SLOW TX] ${agentName(pack.objHash)} - ${pack.service}"
    ...
    private fun levelColor(levelName: String): String =
        when (levelName.uppercase()) {
            "FATAL" -> "#A32D2D"
            "ERROR" -> "#D95C3F"
            "WARN" -> "#D9B300"
            else -> "#185FA5" // INFO
